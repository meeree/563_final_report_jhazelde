...
#         pdb.set_trace()
    ...
    def generate(self, x, u=None):
        ...
        self.loss_record = np.zeros(self.T)
        ...
        for t in range(self.T):
            ...
            # Reconstruction loss
            self.loss_record[t] = self.logLikelihood(x[0, t-1, 0], self.r[0, 0] * self.dt)
        ...
        # plt.subplot(3,1,2)
        # plt.plot(self.rates[0, :, 0].numpy())
        # plt.ylabel('r(t)')
        # plt.xticks([])
        
        # plt.subplot(3,1,3)
        # plt.plot(self.loss_record)
        # plt.ylabel('LogLikelihood(t)')
        # plt.xticks([0, self.T], [0.0, 1.0])
        # plt.xlabel('Time (s)')
        # plt.tight_layout()
    ...
    def plot_summary(self, data, truth=None, num_average=100):
        ...
        return figs_dict, true_data, pred_data
