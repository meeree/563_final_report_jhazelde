...
class BNN(nn.Module):
    def __init__(self, input_dim, output_dim):
        ...
        for d1, d2 in zip(self.dims[:-1], self.dims[1:]):
            self.Ws.append(nn.Linear(d1, d2, bias=True))
            if CFG.use_DNN:
...
class HH_Synaptic(nn.Module):
    ...
    def forward(self, z):
        ...
        h = torch.zeros((B, N, self.L)).to('cuda')
...
class HH_Gap(nn.Module):
    ...
    def forward(self, z):
        ...
        pow1 = torch.zeros((B, self.L)).to('cuda')
        pow2 = torch.zeros((B, self.L)).to('cuda')  
        self.V = torch.ones((B, N, self.L)).to('cuda') * -65.0
        noise = torch.randn((B, N, self.L)).to('cuda') * 0
        ...
        for k in range(1, N):
           ...
           self.V[:, k, :] = (self.V[:, k-1, :].clone() * (1 - G_scaled) + dt * (E + Iapp + noise[:, k, :] + z[:, k-1, :])) / (1 + G_scaled)
